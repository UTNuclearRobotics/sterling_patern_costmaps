# Commands to run inside the container

### Step 1: Choose the terrain resolution you want ot run the Gazebo world
alias run_gazebo_low='roslaunch sterling_gazebo sidewalks.launch.py high_res:=false namespace:=panther'
alias run_gazebo_high='roslaunch sterling_gazebo sidewalks.launch.py high_res:=true namespace:=panther'

### Step 2: Launch Nav2
# Sterling parameters
alias run_nav2='roslaunch utexas_panther bringup_launch.py namespace:=panther observation_topic:=ouster/scan observation_topic_type:=laserscan slam:=true use_rviz:=True use_sim_time:=true params_file:=/root/ros_ws/src/sterling_patern_costmaps/config/nav2_params.yaml'

# How to send a goal_pose now and the alias does not work
function run_goal() {
    local x=${1:-0.0}
    local y=${2:-0.0}
    ros2 topic pub /panther/goal_pose geometry_msgs/msg/PoseStamped "{ header: { frame_id: 'panther/map' }, pose: { position: { x: $x, y: $y }, orientation: { x: 0, y: 0, z: 0, w: 1 } } }"
}

### Step 3: Sterling costmap node
alias run_sterling_patern_costmap='source /root/ros_ws/install/setup.bash && ros2 launch sterling_patern_costmaps costmaps.launch.py'
# While node is running, call service to save the costmap
alias save_costmap='source /root/ros_ws/install/setup.bash && ros2 service call /sterling_patern_costmaps/save_costmap std_srvs/srv/Trigger'